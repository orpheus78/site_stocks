...
>
> Os nomes técnicos mantêm-se inalterados (tabelas `vendas`/`venda_itens`, rotas
> `/api/vendas` e `/admin/vendas`). A mudança é de **linguagem na interface**.
...
**POS — Movimentos Internos (registo rápido)**
...
- Criação/edição de artigos com preço, categoria, ordem, imagem e stock inicial.
...
- Dois perfis: `admin` (POS + caixa + gestão) e `funcionario` (só POS).
...
> **não altera** uma tabela `vendas` que já exista. Para os movimentos internos foi
...
>   -e "SHOW COLUMNS FROM vendas LIKE 'metodo_pagamento';"
> ```
>
...
| --- | :---: | :---: |
| POS — ecrã de Movimentos Internos (`/pos`) | ✅ | ✅ |
| POS — catálogo e registo (`/api/pos/artigos`, `POST /api/vendas`) | ✅ | ✅ |
| POS — comprovativo (`/pos/venda/:id/talao`) | ✅ | ✅ |
...
| Backoffice — movimentos internos e anulações (`/admin/vendas`) | ✅ | ❌ 403 |

Como se comporta na prática:

- **Depois do login**, cada perfil vai para a sua área: o `admin` para `/admin`, o
  funcionário **sempre** para `/pos`. Se um funcionário tentar abrir `/caixa` sem sessão,
  é levado ao login e, depois de entrar, vai para o POS — nunca para um ecrã de 403.
...
- **Com sessão de funcionário** numa área de admin, a resposta é **`403`** (página de erro
  com botão de volta ao POS) ou `403` em JSON nas rotas `/api/*`.
- **O menu esconde** o que o perfil não pode abrir: o funcionário só vê *Movimentos Internos*
  e *Sair*, e no POS não lhe aparecem os atalhos de *Caixa* e *Gestão*.
...
2. **O funcionário regista os movimentos** no POS durante todo o dia. Não precisa de saber
...
| Acesso | Tudo: POS, caixa e backoffice | Apenas o POS |
...
A partir daqui, os movimentos internos registados no POS ficam associados a esta sessão de
...
### 3. Registar um movimento interno no POS *(ambos)*

1. Tocar em **Movimentos Internos** no menu (ou ir a `/pos`).
...
O POS assinala sozinho os artigos que estão no ou abaixo do **stock mínimo** definido no
backoffice:
...
- **No POS**, quando não há caixa aberta aparece uma faixa de aviso: *"Não há caixa aberta
...
3. Registar um movimento:
   ...
   - **Saída** — saiu mercadoria sem passar pelo POS (quebra, oferta): subtrai.
...
- **Gestão → Relatórios** — escolher um intervalo de datas (por omissão, os últimos 30 dias)
  e ver totais, movimentos por dia, os 15 artigos mais consumidos e o consumo por categoria.
...
`público` = não exige sessão · `POS` = sessão iniciada, admin ou funcionário
(`requirePos`) · `admin` = sessão iniciada **com perfil admin** (`requireAdmin`).
...
| --- | --- | --- | --- |
| GET | `/` | público | Encaminha para a área do perfil: `/admin` (admin), `/pos` (funcionário) ou `/login` (sem sessão). |
...
| POST | `/pos/pin` | público | Entrada rápida por PIN de 4 dígitos. Vai sempre para `/pos`. |
...
| Método | Caminho | Autenticação | Descrição |
| --- | --- | --- | --- |
| GET | `/pos` | POS | Ecrã de **Movimentos Internos** (táctil). |
| GET | `/api/pos/artigos` | POS | Catálogo JSON: categorias e artigos ativos com preço, stock e alerta de stock baixo (ver contrato abaixo). |
| POST | `/api/vendas` | POS | Regista um movimento interno (ver contrato abaixo). |
| GET | `/pos/venda/:id/talao` | POS | Comprovativo HTML imprimível. `?papel=58` para 58 mm; qualquer outro valor usa 80 mm. |

> Os caminhos `/api/vendas` e `/pos/venda/:id/talao` mantêm o nome técnico de propósito:
> renomear rotas seria uma mudança de risco sem benefício para o utilizador. A alteração
> é apenas de linguagem na interface.
...
| GET | `/admin/vendas` | admin | Lista de movimentos internos (`?de=&ate=&estado=&metodo=`). |
| GET | `/admin/vendas/:id` | admin | Detalhe do movimento. |
| POST | `/admin/vendas/:id/anular` | admin | Anula o movimento e repõe o stock. |

---

## Contrato `GET /api/pos/artigos`

Catálogo do POS. Exige sessão iniciada; sem sessão devolve `401` em JSON.
...
O POS usa este campo para marcar o artigo com um aviso visível (âmbar quando está a
...
`venda.avisosStock`) e **`avisos_stock`** é a mesma informação estruturada, com o campo
...
| `tipo` | Quando | Como o POS o mostra |
| --- | --- | --- |
...
| `400` | Regra de negócio dos **métodos antigos**: valor entregue inferior ao total, ou misto sem uma das componentes. Nunca ocorre em `interno`. | `{ "erro": "Valor entregue inferior ao total da venda." }` |
...
erDiagram
    ...
    UTILIZADORES ||--o{ VENDAS : "efetua"
    CATEGORIAS ||--o{ ARTIGOS : "agrupa"
    ARTIGOS ||--|| STOCKS : "tem"
    ARTIGOS ||--o{ MOVIMENTOS_STOCK : "movimenta"
    ARTIGOS ||--o{ VENDA_ITENS : "vendido em"
    SESSOES_CAIXA ||--o{ MOVIMENTOS_CAIXA : "contem"
    SESSOES_CAIXA ||--o{ VENDAS : "agrega"
    VENDAS ||--|{ VENDA_ITENS : "composta por"
    ...
    MOVIMENTOS_STOCK {
        int id PK
        int artigo_id FK
        enum tipo "entrada|saida|ajuste|venda"
        decimal quantidade
...
`funcionario`: o `admin` acede a tudo, o `funcionario` **apenas ao POS** (ver
[Perfis e permissões](#perfis-e-permissões)). `ativo = 0` impede o login.

#### `categorias`
Agrupamento dos artigos no POS. `cor` (hexadecimal `#rrggbb`) e `ordem` controlam a
...
eliminada (`ON DELETE SET NULL`). `ordem` define a posição na grelha do POS.
...
(inventário: define o valor absoluto) ou `venda`. `quantidade_apos` guarda o stock
...
O balcão passou a registar **consumo interno**, não vendas: escolhem-se os artigos e
...
[`caixa.repo.totaisVendas`](src/repositories/caixa.repo.js):
...
visíveis**: aviso no POS quando não há caixa aberta e aviso destacado em `/caixa`, com a
...
As tabelas continuam a chamar-se `vendas`/`venda_itens`, as rotas `/api/vendas` e
`/admin/vendas`, e as funções `criarVenda`/`anularVenda`. Renomear código, rotas e schema
...
`POST /api/vendas` aceita na mesma `dinheiro`, `multibanco` e `misto`, com todas as regras
...
artigo que aparece em `venda_itens` limita-se a colocar `ativo = 0`: desaparece do POS mas
...
reforço, `venda_itens` guarda `nome_snapshot` e `preco_unit`, pelo que o histórico sobrevive
...
O POS é um browser: tudo o que envia pode ser manipulado. O pedido transporta apenas
...
O número vem de `SELECT COALESCE(MAX(numero), 0) + 1 FROM vendas FOR UPDATE`, executado
...
segundo `INSERT` falharia contra o índice único `uq_vendas_numero`. O movimento completo —
...
| `vendas.calcularPagamento.test.js` | **Movimento interno** (sem método no payload, sem troco, nunca recusado) e, no histórico, métodos de pagamento, valor certo, valor insuficiente e a regra do troco só sobre dinheiro. |
| `vendas.carrinho.test.js` | Agregação de itens repetidos, validação de artigos e quantidades, subtotais e total. |
| `stock.calculo.test.js` | Nova quantidade por tipo de movimento (incluindo o ajuste absoluto), deteção de stock negativo e de stock baixo, e a derivação de `stock_baixo` para o catálogo do POS (stock nulo, zero, negativo, igual ao mínimo e acima do mínimo). |
| `vendas.avisosStock.test.js` | Classificação dos avisos devolvidos por `POST /api/vendas`: `stock_baixo` vs `stock_negativo`, limite inclusivo e um único aviso por artigo. |
...
| `guards.test.js` | Rotas operacionais (`/health`, `/ready`) e proteção das rotas de POS, caixa e backoffice. |
| `permissoes.test.js` | Separação de perfis: funcionário recebe `403` em todo o `/admin/*` e `/caixa/*` (HTML e JSON) mas passa no POS; admin acede a tudo; sem sessão continua a redirecionar para `/login`; redirect por perfil após login e em `GET /`. Confirma também que o POS **não depende da caixa** nem mostra mecânica de dinheiro. |
| `vendas-api.test.js` | Validação do payload de `POST /api/vendas`, o **movimento interno sem `metodo_pagamento`** (201, `interno`, dinheiro e troco a zero, valores de dinheiro enviados por engano ignorados) e a confirmação de que o preço usado é o da base de dados, não o enviado pelo cliente. |
| `pos-alertas-stock.test.js` | `GET /api/pos/artigos` devolve `stock_minimo` e `stock_baixo` corretos (incluindo artigos sem linha de stock) e `POST /api/vendas` devolve `avisos_stock` com o `tipo` certo sem nunca bloquear a venda. |
...
relatórios, a **separação de perfis** (passo 11), os **alertas de stock baixo no POS**
...
`GET /pos/venda/:id/talao` é uma página HTML formatada para papel de **80 mm**. Para
impressoras de **58 mm**, acrescentar `?papel=58` ao endereço; qualquer outro valor do
parâmetro usa 80 mm. A impressão é feita pela função de imprimir do navegador. Não é um
talão de venda: não tem método de pagamento, valor recebido nem troco.
...
1. **Havia caixa aberta quando os movimentos foram registados?** Movimentos gravados com a
   ...
   ```sql
   SELECT COUNT(*), SUM(total) FROM vendas
...
src/
  ...
  routes/        index auth pos caixa admin categorias artigos stocks vendas
  controllers/   auth pos caixa categorias artigos stocks vendas relatorios
  services/      auth vendas stock caixa relatorios AppError
  repositories/  base utilizadores categorias artigos stocks movimentosStock caixa vendas relatorios
views/           layouts/ partials/ auth/ pos/ caixa/ admin/ errors/
