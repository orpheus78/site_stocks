/* ==========================================================================
   MOVIMENTOS INTERNOS — logica do ecra de registo (antigo POS).
...
(function () {
  ...
  var raiz = document.getElementById('pos');
  if (!raiz) return;
  ...
  var elCategorias = el('posCategorias');
  var elGrelha = el('posGrelha');
  var elEstado = el('posEstado');
  var elPesquisa = el('posPesquisa');
  var elPesquisaLimpar = el('posPesquisaLimpar');

  var elLinhas = el('posLinhas');
  var elCarrinhoVazio = el('posCarrinhoVazio');
  var elTotal = el('posTotal');
  var elContagem = el('posContagem');
  var elRegistar = el('posRegistar');
  var elLimpar = el('posLimpar');

  var elModal = el('posModalMovimento');
  var elModalTotal = el('posModalTotal');
  var elResumo = el('posResumo');
  var elMovimentoErro = el('posMovimentoErro');
  var elConfirmar = el('posConfirmar');

  var elSucesso = el('posSucesso');
  var elSucessoNumero = el('posSucessoNumero');
  var elSucessoNova = el('posSucessoNova');

  var elToasts = el('posToasts');
  var elRelogio = el('posRelogio');

  var elFaltaBtn = el('posFaltaBtn');
  var elFaltaTexto = el('posFaltaTexto');
  var elFaltaPainel = el('posFaltaPainel');
  var elFaltaLista = el('posFaltaLista');
  var elFaltaFechar = el('posFaltaFechar');
  ...
  function toast(mensagem, tipo) {
    ...
    div.className = 'pos-toast pos-toast-' + (tipo || 'info');
      ...
      .catch(function (err) {
        ...
        mostrarEstado(
          '<p><i class="bi bi-wifi-off"></i> Nao foi possivel carregar os artigos.</p>' +
            '<button type="button" class="pos-estado-btn" id="posTentarNovamente">Tentar novamente</button>'
        );
        var btn = el('posTentarNovamente');
        if (btn) {
  ...
  function botaoCategoria(id, nome, cor) {
    ...
    btn.type = 'button';
    btn.className = 'pos-categoria' + (estado.categoriaAtiva === id ? ' is-ativa' : '');
  ...
  /**
   * Classifica o artigo para o alerta visual do POS.
  ...
  function desenharListaFalta(lista) {
    ...
    lista.forEach(function (artigo) {
      ...
      li.className = 'pos-falta-item' + (alertaStock(artigo) === 'esgotado' ? ' is-esgotado' : '');

      var nome = document.createElement('span');
      nome.className = 'pos-falta-nome';
  ...
  function criarTile(artigo) {
    ...
    if (alerta) {
      ...
      stock.className = 'pos-tile-stock is-' + alerta;
    ...
    texto.className = 'pos-tile-texto';

    var nome = document.createElement('span');
    nome.className = 'pos-tile-nome';
    nome.textContent = artigo.nome;

    var preco = document.createElement('span');
    preco.className = 'pos-tile-preco';
  ...
  function criarLinha(linha, destacar) {
    var li = document.createElement('li');
    li.className = 'pos-linha' + (destacar ? ' is-nova' : '');

    var info = document.createElement('div');
    info.className = 'pos-linha-info';

    var nome = document.createElement('div');
    nome.className = 'pos-linha-nome';
    nome.textContent = linha.artigo.nome;

    var unit = document.createElement('div');
    unit.className = 'pos-linha-unit';
    ...
    subtotal.className = 'pos-linha-subtotal';
    subtotal.textContent = eur(linha.artigo.preco * linha.quantidade);

    var acoes = document.createElement('div');
    acoes.className = 'pos-linha-acoes';

    var menos = document.createElement('button');
    menos.type = 'button';
    menos.className = 'pos-qtd-btn';
    menos.textContent = '−';
  ...
  function desenharResumo() {
    ...
    estado.carrinho.forEach(function (linha) {
      var li = document.createElement('li');
      li.className = 'pos-resumo-item';

      var nome = document.createElement('span');
      nome.className = 'pos-resumo-nome';
      nome.textContent = linha.quantidade + 'x ' + linha.artigo.nome;

      var valor = document.createElement('span');
      valor.className = 'pos-resumo-valor';
  ...
  function mostrarSucesso(dados) {
    ...
    elSucessoNumero.textContent =
      'Movimento #' + (venda.numero || venda.id || '') + ' · ' + eur(venda.total || 0);
  ...
  elLimpar.addEventListener('click', function () {
    ...
    // Painel de confirmacao proprio do POS (/js/confirmar.js) — nada de
  ...
  el('posModalFechar').addEventListener('click', fecharMovimento);
  el('posCancelar').addEventListener('click', fecharMovimento);
