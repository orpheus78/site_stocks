...
describe('POST /api/consumos — validacao de payload', () => {
  let agent;

  before(async () => {
    const { app } = loadAppWithFakeDb(handlersConsumosApi());
  ...
  test('Dado um carrinho vazio, responde 422 com erro estruturado', async () => {
    const res = await agent.post('/api/consumos').send({ itens: [], metodo_pagamento: 'dinheiro' });
  ...
  test('Dado um valor_dinheiro negativo, responde 422 com erro estruturado', async () => {
    const res = await agent.post('/api/consumos').send({
...
describe('POST /api/consumos — movimento interno (sem metodo_pagamento)', () => {
  let agent;
  let insercoes;

  before(async () => {
    insercoes = [];
    const handlers = handlersConsumosApi().map((h) =>
      /INSERT INTO consumos/i.test(h.pattern.source)
        ? {
            pattern: h.pattern,
            handler: (params) => {
  ...
  test('Sem metodo_pagamento no payload, grava como interno com troco e valores a zero (201)', async () => {
    ...
    assert.equal(res.body.consumo.total, 3); // 2 x 1.50
    assert.equal(res.body.consumo.metodo_pagamento, 'interno');
...
describe('POST /api/consumos — precos vem sempre da BD, nunca do cliente', () => {
  let agent;
  ...
  test('Dado um preco adulterado enviado pelo cliente, o consumo usa sempre o preco real da BD', async () => {
    ...
    // When
    const res = await agent.post('/api/consumos').send(payload);
