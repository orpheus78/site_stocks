...
/**
 * Alertas de stock baixo no GIM (requisito do cliente):
 * "criar um alerta caso o stock do artigo esteja abaixo do stock minimo, na
 * parte do consumo, assim o utilizador pode avisar o responsavel".
 *
 * Cobre o contrato HTTP das duas pecas:
 *   - GET  /api/gim/artigos -> `stock_minimo` + booleano derivado `stock_baixo`
 *   - POST /api/consumos      -> `avisos` (texto) e `avisos_stock` (com `tipo`)
 *
 * REGRA DE NEGOCIO: o consumo NUNCA e bloqueado por falta de stock. Os avisos
...
function handlers() {
  return [
    ...
    { pattern: /INSERT INTO consumo_itens/i, handler: () => ({ insertId: 1 }) },
...
async function doCatalogo(nome) {
  const res = await agente.get('/api/gim/artigos');
...
describe('GET /api/gim/artigos — stock_minimo e stock_baixo', () => {
...
describe('POST /api/consumos — avisos de stock com tipo', () => {
  test('Dado um consumo que empurra o artigo para baixo do minimo, devolve aviso do tipo stock_baixo (201)', async () => {
    ...
    // Then: o consumo passa (nunca e bloqueado) e traz o aviso classificado
  ...
  test('Dado um consumo que deixa o stock negativo, o aviso e do tipo stock_negativo (nao duplica)', async () => {
  ...
  test('Dado um consumo sem impacto no minimo, nao ha avisos nenhuns', async () => {
  ...
  test('Dado um consumo com varios artigos, cada um traz o seu aviso com o tipo correto', async () => {
    const res = await agente.post('/api/consumos').send({
      itens: [
