...
describe('caixa.service.calcularResumo', () => {
  test('CASO DO CLIENTE: fundo 20 + movimentos internos de 5 => esperado 25', () => {
    ...
    const totais = { dinheiro: 0, multibanco: 0, interno: 5, total: 5, n_consumos: 2 };
  ...
  test('Dada uma sessao sem movimentos e sem consumos, o esperado e igual ao fundo inicial', () => {
    // Given
    const sessao = { fundo_inicial: 50 };
    const totais = { dinheiro: 0, multibanco: 0, interno: 0, total: 0, n_consumos: 0 };
  ...
  test('Dadas vendas antigas em dinheiro, o esperado inclui o dinheiro recebido mas NAO o multibanco', () => {
    // Given: fundo de 20, consumos com 30 em dinheiro (liquido de troco) e 100 em multibanco
    const sessao = { fundo_inicial: 20 };
    const totais = { dinheiro: 30, multibanco: 100, interno: 0, total: 130, n_consumos: 5 };
    ...
    assert.equal(resumo.consumos_multibanco, 100);
  });
  ...
  test('Dado um cenario completo (fundo + consumos dinheiro + entradas - saidas - sangrias), calcula o esperado corretamente', () => {
    // Given
    const sessao = { fundo_inicial: 50 };
    const movimentos = [
    ...
    const totais = { dinheiro: 80, multibanco: 200, interno: 0, total: 280, n_consumos: 12 };
  ...
  test('Movimentos internos entram no valor esperado em caixa pelo seu total', () => {
    ...
    const sessao = { fundo_inicial: 50 };
    const totais = { dinheiro: 0, multibanco: 0, interno: 137.5, total: 137.5, n_consumos: 9 };
    ...
    assert.equal(resumo.consumos_total, 137.5);
    assert.equal(resumo.n_consumos, 9);
  });

  test('Movimentos internos misturados com vendas antigas em dinheiro: contam os dois', () => {
    // Given: 40 em dinheiro de consumos historicas + movimentos internos que somam 60
    const sessao = { fundo_inicial: 25 };
    const totais = { dinheiro: 40, multibanco: 0, interno: 60, total: 100, n_consumos: 7 };
  ...
  test('Multibanco de vendas antigas continua fora do esperado, mesmo com movimentos internos', () => {
    ...
    const sessao = { fundo_inicial: 20 };
    const totais = { dinheiro: 0, multibanco: 100, interno: 5, total: 105, n_consumos: 3 };
  ...
  test('Dados valores com centimos vindos de varios movimentos, o esperado nao acumula erro de virgula flutuante', () => {
    ...
    const totais = { dinheiro: 0, multibanco: 0, interno: 0.1, total: 0.1, n_consumos: 1 };
